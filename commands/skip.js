...
module.exports.run = async (bot, message, args, ops) => {
    ...
    if (!guildIDData) return message.channel.send("There is at the moment no music playing.");
 
    // Nakijken als in zelfde kanaal zit als de bot.
    if (message.member.voiceChannel !== message.guild.me.voiceChannel) return message.channel.send("Please connect to the same voice channel as the bot.");
    ...
    message.channel.send(`Added of skip request. ${guildIDData.queue[0].voteSkips.length}/${amountSkip}`);
