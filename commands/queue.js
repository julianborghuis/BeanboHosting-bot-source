...
module.exports.run = async (bot, message, args, ops) => {
    ...
    if (!args[0]) return message.channel.send("Sorry, give me a URL");
 
    var validate = await ytdl.validateURL(args[0]);
 
    if (!validate) return message.channel.send("Give me a valid URL");
    ...
    if (!data.dispatcher) {
        ...
        message.channel.send(`Added to the queue: ${info.title} | Asked by: ${message.author.tag}`);
